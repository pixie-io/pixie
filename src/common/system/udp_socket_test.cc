...
TEST(UDPSocketTest, SendToAndRecvFrom) {
  ...
  EXPECT_EQ(5, client.SendTo("pixie", server)) << absl::Substitute("errno=$0", errno);
...
TEST(UDPSocketTest, AddrAndPort) {
  ...
  {
    client.SendTo("foo", server);

    std::string data;
    std::unique_ptr<UDPSocket> source = server.RecvFrom(&data);

    ASSERT_NE(source.get(), nullptr);

    auto client_in_addr = source->addr();
    ...
    uint16_t client_port = ntohs(source->port());
