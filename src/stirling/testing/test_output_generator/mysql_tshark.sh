...
tshark -i any -O mysql -w "$PIXIE_ROOT"/src/stirling/mysql/testing/raw.pcap -q > /dev/null&
TSHARK_PID=$!
...
tshark -i any -2 -r "$PIXIE_ROOT"/src/stirling/mysql/testing/raw.pcap -R mysql -T json \
> "$PIXIE_ROOT"/src/stirling/mysql/testing/tshark.json
