...
void RRSingleton::RecordBPFArrayTableGetValueEvent(const std::string& name, const int32_t idx,
...
void RRSingleton::WriteProto(const std::string& proto_buf_file_path) {
...
Status RRSingleton::ReplayArrayGetValue(const std::string& name, const int32_t idx, void* value,
  ...
  memcpy(value, array_table_get_value_event.data().data(), data_size);
  ++playback_event_idx_;

  return Status::OK();
}

void RRSingleton::SetRecording() {
  ECHECK(!replaying_);
  recording_ = true;
}

void RRSingleton::SetReplaying(const std::string& proto_buf_file_path) {
  ECHECK(!recording_);
  replaying_ = true;

  std::fstream input(proto_buf_file_path, std::ios::in | std::ios::binary);

  if (!input) {
    ECHECK(false) << "Could not open file: " << proto_buf_file_path << ".";
  } else if (!events_proto_.ParseFromIstream(&input)) {
    ECHECK(false) << "Could not parse file: " << proto_buf_file_path << ".";
  }
}

void RecordPerfBufferEvent(void* cb_cookie, void* data, int data_size) {
  auto event = RRSingleton::GetInstance().events_proto().add_event()->mutable_perf_buffer_event();
  auto pb_name = event->mutable_name();
  auto pb_data = event->mutable_data();

  const std::string data_as_string(static_cast<char*>(data), data_size);
