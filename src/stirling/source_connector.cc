...
Status SourceConnector::Init() {
  if (state_ != ConnectorState::kUninitialized) {
    return error::Internal("Cannot re-initialize a connector.");
  }
  Status s = InitImpl();
  state_ = s.ok() ? ConnectorState::kActive : ConnectorState::kErrors;
  return s;
}
...
Status SourceConnector::Stop() {
  if (state_ != ConnectorState::kActive) {
    return error::Internal("Cannot stop connector that is not active.");
  }
  Status s = StopImpl();
  state_ = s.ok() ? ConnectorState::kStopped : ConnectorState::kErrors;
  return s;
}
