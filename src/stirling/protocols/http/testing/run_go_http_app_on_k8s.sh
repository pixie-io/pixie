...
sed "s/{{USER}}/${USER}/" src/stirling/protocols/http/testing/go_http_server/deployment.yaml | \
  kubectl apply -f -
sed "s/{{USER}}/${USER}/" src/stirling/protocols/http/testing/go_http_client/deployment.yaml | \
  kubectl apply -f -
