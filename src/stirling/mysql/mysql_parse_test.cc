...
TEST_F(MySQLParserTest, ParseComStmtClose) {
  ...
  std::deque<Packet> parsed_messages;
  ParseResult result = ParseFrame(MessageType::kRequest, msg, &parsed_messages);
...
TEST_F(MySQLParserTest, ParseResponse) {
  ...
  std::deque<Packet> parsed_messages;
  ParseResult result =
      ParseFrame(MessageType::kResponse, kMySQLStmtPrepareMessage.response, &parsed_messages);
  EXPECT_EQ(ParseState::kSuccess, result.state);
