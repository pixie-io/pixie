...
pixielabs!)";
  ...
  ParseResult result = parser_.ParseMessages(kMessageTypeResponses, &parsed_messages);
...
TEST(ParseTest, CompleteMessages) {
  ...
  std::deque<HTTPMessage> parsed_messages;
  ParseResult result = Parse(kMessageTypeResponses, buf, &parsed_messages);
...
TEST(ParseTest, PartialMessages) {
  ...
  std::deque<HTTPMessage> parsed_messages;
  ParseResult result = Parse(kMessageTypeResponses, msg, &parsed_messages);
...
TEST_F(HTTPParserStressTest, ParseHTTPRequestSingle) {
  ...
  std::deque<HTTPMessage> parsed_messages;
  ParseResult result = parser_.ParseMessages(kMessageTypeRequests, &parsed_messages);
...
TEST_P(HTTPParserStressTest, ParseHTTPResponsesWithLeftoverRepeatedly) {
  ...
  for (uint32_t j = 0; j < GetParam().iters; ++j) {
    ...
    ParseResult result1 = parser_.ParseMessages(kMessageTypeResponses, &parsed_messages);
    ...
    ParseResult result2 = parser_.ParseMessages(kMessageTypeResponses, &parsed_messages);
