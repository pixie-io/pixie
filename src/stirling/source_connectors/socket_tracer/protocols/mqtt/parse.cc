  ...
  while (properties_length > 0) {
    ...
    switch (property_code) {
      ...
      case static_cast<uint8_t>(PropertyCode::ContentType): {
        PX_ASSIGN_OR_RETURN_INVALID(uint16_t property_length,
      ...
      case static_cast<uint8_t>(PropertyCode::ServerKeepAlive): {
        PX_ASSIGN_OR_RETURN_INVALID(uint16_t server_keep_alive,
                                            decoder->ExtractBEInt<uint16_t>());
      ...
      case static_cast<uint8_t>(PropertyCode::ReceiveMaximum): {
        PX_ASSIGN_OR_RETURN_INVALID(uint16_t receive_maximum,
                                            decoder->ExtractBEInt<uint16_t>());
      ...
      case static_cast<uint8_t>(PropertyCode::TopicAlias): {
        PX_ASSIGN_OR_RETURN_INVALID(uint16_t topic_alias,
                                            decoder->ExtractBEInt<uint16_t>());
      ...
      case static_cast<uint8_t>(PropertyCode::RetainAvailable): {
        PX_ASSIGN_OR_RETURN_INVALID(uint8_t retain_available,
                                            decoder->ExtractBEInt<uint8_t>());
      ...
      case static_cast<uint8_t>(PropertyCode::UserProperty): {
        ...
        properties_length -= 2;
        PX_ASSIGN_OR_RETURN_INVALID(std::string_view key,
                                            decoder->ExtractString(key_length));
        properties_length -= key_length;
        PX_ASSIGN_OR_RETURN_INVALID(uint16_t value_length,
                                            decoder->ExtractBEInt<uint16_t>());
        properties_length -= 2;
        PX_ASSIGN_OR_RETURN_INVALID(std::string_view value,
...
ParseState ParseVariableHeader(Message* result, BinaryDecoder* decoder,
  ...
  switch (control_packet_type) {
    case MqttControlPacketType::CONNECT: {
      PX_ASSIGN_OR_RETURN_INVALID(uint16_t protocol_name_length,
                                          decoder->ExtractBEInt<uint16_t>());
      ...
      PX_ASSIGN_OR_RETURN_INVALID(uint8_t protocol_version,
                                          decoder->ExtractBEInt<uint8_t>());
...
ParseState ParsePayload(Message* result, BinaryDecoder* decoder,
  ...
  switch (control_packet_type) {
    case MqttControlPacketType::CONNECT: {
      PX_ASSIGN_OR_RETURN_INVALID(uint16_t client_id_length,
                                          decoder->ExtractBEInt<uint16_t>());
      ...
      if (result->header_fields["username_flag"]) {
        PX_ASSIGN_OR_RETURN_INVALID(size_t username_length,
                                            decoder->ExtractBEInt<uint16_t>());
      ...
      if (result->header_fields["password_flag"]) {
        PX_ASSIGN_OR_RETURN_INVALID(size_t password_length,
                                            decoder->ExtractBEInt<uint16_t>());
    ...
    case MqttControlPacketType::PUBLISH: {
      ...
      PX_ASSIGN_OR_RETURN_INVALID(std::string_view payload,
