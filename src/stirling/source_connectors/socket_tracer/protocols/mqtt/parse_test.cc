...
TEST_F(MQTTParserTest, Properties) {
    ...
    uint8_t response_topic_publish[] = {
            ...
            0x08, 0x00, 0x06, 0x41, 0x42, 0x43, 0x58,
            0x59, 0x5a,
            // message
            0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x20, 0x77,
            0x6f, 0x72, 0x6c, 0x64};
    uint8_t correlation_data_publish[] = {
            // header flags
            0x30,
            // message length
            0x21,
            ...
            0x09, 0x00, 0x06, 0x41, 0x42, 0x43, 0x58,
            0x59, 0x5a,
            // message
            0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x20, 0x77,
            0x6f, 0x72, 0x6c, 0x64};
    uint8_t subscription_id_subscribe[] = {
            // header flags
            0x82,
            // message length
            0x15,
            ...
            0x00};
    uint8_t session_exp_int_recv_max_connect[] = {
            // header flags
            0x10,
            // message length
            0x15,
    ...
    uint8_t assigned_cid_topic_alias_max_recv_max_connack[] = {
            ...
            0x12, 0x00, 0x29, 0x61, 0x75, 0x74,
            0x6f, 0x2d, 0x43, 0x38, 0x30, 0x36,
            0x33, 0x38, 0x36, 0x38, 0x2d, 0x37,
            0x38, 0x30, 0x34, 0x2d, 0x33, 0x46,
            0x36, 0x36, 0x2d, 0x30, 0x36, 0x42,
            0x38, 0x2d, 0x42, 0x41, 0x43, 0x44,
            0x39, 0x46, 0x36, 0x37, 0x33, 0x43,
            0x42, 0x30,
            // receive maximum code and value
            0x21, 0x00, 0x14};
    uint8_t auth_method_data_connect[] = {
            // header flags
            0x10,
            // message length
            0x37,
            ...
            0x15, 0x00, 0x0d, 0x53, 0x43, 0x52, 0x41,
            0x4d, 0x2d, 0x53, 0x48, 0x41, 0x2d, 0x32,
            0x35, 0x36,
            // authentication data code, length and value
            0x16, 0x00, 0x14, 0x63, 0x6c, 0x69, 0x65,
            0x6e, 0x74, 0x2d, 0x66, 0x69, 0x72, 0x73,
            0x74, 0x2d, 0x6d, 0x65, 0x73, 0x73, 0x61,
            0x67, 0x65,
            // receive maximum code and value
            0x21, 0x00, 0x14,
            // client id length
            0x00, 0x00};
    uint8_t req_prob_info_connect[] = {
            // header flags
            0x10,
            // message length
            0x12,
            ...
            // client id length
            0x00, 0x00};
    uint8_t will_delay_interval_connect[] = {
            // header flags
            0x10,
            // message length
            0x2b,
            ...
            0x00, 0x0a, 0x77, 0x69, 0x6c, 0x6c,
            0x2d, 0x74, 0x6f, 0x70, 0x69, 0x63,
            // will message length and value
            0x00, 0x07, 0x67, 0x6f, 0x6f, 0x64, 0x62,
            0x79, 0x65};
    uint8_t req_resp_info_connect[] = {
            // header flags
            0x10,
            // message length
            0x12,
    ...
    uint8_t user_prop_subscribe[] = {
            ...
            // subscription options
            0x00};
    uint8_t max_packet_size_connect[] = {
            // header flags
            0x10,
            // message length
            0x15,
...
TEST_F(MQTTParserTest, Payload) {
    ...
    uint8_t kConnectFrame[] = {
            ...
            0x18, 0x00, 0x00, 0x00, 0x1e,
            // will topic length and value
            0x00, 0x0a, 0x77, 0x69, 0x6c, 0x6c, 0x2d,
            0x74, 0x6f, 0x70, 0x69, 0x63,
            // will message length and value
            0x00, 0x07, 0x67, 0x6f, 0x6f, 0x64, 0x62,
            0x79, 0x65,
            // username length and value
            0x00, 0x09, 0x64, 0x75, 0x6d, 0x6d, 0x79,
            0x75, 0x73, 0x65, 0x72,
            // password length and value
            0x00, 0x09, 0x64, 0x75, 0x6d, 0x6d, 0x79,
            0x70, 0x61, 0x73, 0x73};
    uint8_t kPublishFrame[] = {
            // header flags
            0x32,
            // message length
            0x1a,
            // topic length
            0x00, 0x0a,
            0x74, 0x65, 0x73, 0x74, 0x2f, 0x74, 0x6f, 0x70,
            0x69, 0x63,
            // message identifier
            0x00, 0x01,
            // properties length
    ...
    EXPECT_EQ(frame.payload["password"], "dummypass");
    frame = Message();
...
TEST_F(MQTTParserTest, Headers) {
    ...
    uint8_t kUnsubackFrame[] = {
            ...
            // reason code
            0x00};
    uint8_t kPingreqFrame[] = {
            // header flags
            0xc0,
            // message length
            0x00};
    uint8_t kPingrespFrame[] = {
            // header flags
            0xd0,
            // message length
            0x00};
    uint8_t kDisconnectFrame[] = {
            // header flags
            0xe0,
            // message length
            0x01,
    ...
    result_state = ParseFrame(message_type_t::kRequest, &frame_view, &frame);
    ASSERT_EQ(result_state, ParseState::kSuccess);
    EXPECT_EQ(frame.control_packet_type, "CONNECT");
    ...
    result_state = ParseFrame(message_type_t::kResponse, &frame_view, &frame);
    ASSERT_EQ(result_state, ParseState::kSuccess);
    EXPECT_EQ(frame.control_packet_type, "CONNACK");
    EXPECT_EQ(frame.header_fields["remaining_length"], (size_t) 53);
    EXPECT_EQ(frame.header_fields["session_present"], (unsigned long) 0);
    EXPECT_EQ(frame.header_fields["reason_code"], (unsigned long) 0);
    frame = Message();

    frame_view = CreateStringView<char>(CharArrayStringView<uint8_t>(kPublishFrame));
    result_state = ParseFrame(message_type_t::kRequest, &frame_view, &frame);
    ASSERT_EQ(result_state, ParseState::kSuccess);
    EXPECT_EQ(frame.control_packet_type, "PUBLISH");
    EXPECT_EQ(frame.header_fields["remaining_length"], (size_t) 26);
    EXPECT_EQ(frame.header_fields["dup"], (unsigned long) 0);
    EXPECT_EQ(frame.header_fields["retain"], (unsigned long) 0);
    EXPECT_EQ(frame.header_fields["qos"], (unsigned long) 1);
    ...
    result_state = ParseFrame(message_type_t::kResponse, &frame_view, &frame);
    ASSERT_EQ(result_state, ParseState::kSuccess);
    EXPECT_EQ(frame.control_packet_type, "PUBACK");
    EXPECT_EQ(frame.header_fields["remaining_length"], (size_t) 3);
    EXPECT_EQ(frame.header_fields["packet_identifier"], (unsigned long) 1);
    frame = Message();

    frame_view = CreateStringView<char>(CharArrayStringView<uint8_t>(kPubrecFrame));
    result_state = ParseFrame(message_type_t::kResponse, &frame_view, &frame);
    ASSERT_EQ(result_state, ParseState::kSuccess);
    EXPECT_EQ(frame.control_packet_type, "PUBREC");
    ...
    result_state = ParseFrame(message_type_t::kRequest, &frame_view, &frame);
    ASSERT_EQ(result_state, ParseState::kSuccess);
    EXPECT_EQ(frame.control_packet_type, "PUBREL");
    EXPECT_EQ(frame.header_fields["remaining_length"], (size_t) 2);
    EXPECT_EQ(frame.header_fields["packet_identifier"], (unsigned long) 1);
    frame = Message();
    ...
    result_state = ParseFrame(message_type_t::kResponse, &frame_view, &frame);
    ASSERT_EQ(result_state, ParseState::kSuccess);
    EXPECT_EQ(frame.control_packet_type, "PUBCOMP");
    EXPECT_EQ(frame.header_fields["remaining_length"], (size_t) 2);
    EXPECT_EQ(frame.header_fields["packet_identifier"], (unsigned long) 1);
    frame = Message();
    ...
    result_state = ParseFrame(message_type_t::kRequest, &frame_view, &frame);
    ASSERT_EQ(result_state, ParseState::kSuccess);
    EXPECT_EQ(frame.control_packet_type, "SUBSCRIBE");
    EXPECT_EQ(frame.header_fields["remaining_length"], (size_t) 16);
    EXPECT_EQ(frame.header_fields["packet_identifier"], (unsigned long) 1);
    frame = Message();
    ...
    result_state = ParseFrame(message_type_t::kResponse, &frame_view, &frame);
    ASSERT_EQ(result_state, ParseState::kSuccess);
    EXPECT_EQ(frame.control_packet_type, "SUBACK");
    ...
    result_state = ParseFrame(message_type_t::kRequest, &frame_view, &frame);
    ASSERT_EQ(result_state, ParseState::kSuccess);
    EXPECT_EQ(frame.control_packet_type, "UNSUBSCRIBE");
    EXPECT_EQ(frame.header_fields["remaining_length"], (size_t) 15);
    EXPECT_EQ(frame.header_fields["packet_identifier"], (unsigned long) 2);
    frame = Message();

    frame_view = CreateStringView<char>(CharArrayStringView<uint8_t>(kUnsubackFrame));
    result_state = ParseFrame(message_type_t::kResponse, &frame_view, &frame);
    ASSERT_EQ(result_state, ParseState::kSuccess);
    EXPECT_EQ(frame.control_packet_type, "UNSUBACK");
    EXPECT_EQ(frame.header_fields["remaining_length"], (size_t) 4);
    EXPECT_EQ(frame.header_fields["packet_identifier"], (unsigned long) 2);
    frame = Message();
    ...
    result_state = ParseFrame(message_type_t::kRequest, &frame_view, &frame);
    ASSERT_EQ(result_state, ParseState::kSuccess);
    EXPECT_EQ(frame.control_packet_type, "PINGREQ");
    ...
    result_state = ParseFrame(message_type_t::kResponse, &frame_view, &frame);
    ASSERT_EQ(result_state, ParseState::kSuccess);
    EXPECT_EQ(frame.control_packet_type, "PINGRESP");
    EXPECT_EQ(frame.header_fields["remaining_length"], (size_t) 0);
    frame = Message();
    ...
    result_state = ParseFrame(message_type_t::kRequest, &frame_view, &frame);
    ASSERT_EQ(result_state, ParseState::kSuccess);
    EXPECT_EQ(frame.control_packet_type, "DISCONNECT");
    EXPECT_EQ(frame.header_fields["remaining_length"], (size_t) 1);
    EXPECT_EQ(frame.header_fields["reason_code"], (unsigned long) 4);
    frame = Message();
}
