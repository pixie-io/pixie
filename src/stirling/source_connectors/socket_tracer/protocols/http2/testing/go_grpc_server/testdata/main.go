...
func (s *server) SayHelloClientStreaming(srv StreamingGreeter_SayHelloClientStreamingServer) error {
	names := []string{}
	for {
		helloReq, err := srv.Recv()
		if err == io.EOF {
			return srv.SendAndClose(&HelloReply{Message: "Hello " + strings.Join(names, ", ") + "!"})
		}
		if err != nil {
			return err
		}
...
func (s *server) SayHelloServerStreaming(in *HelloRequest, srv StreamingGreeter_SayHelloServerStreamingServer) error {
	...
	for i := 0; i < 3; i++ {
		err := srv.Send(&HelloReply{Message: "Hello " + in.Name})
		if err != nil {
			return err
		}
	}
...
func (s *server) SayHelloBidirStreaming(stream StreamingGreeter_SayHelloBidirStreamingServer) error {
	for {
		...
		err = stream.Send(&HelloReply{Message: "Hello " + helloReq.Name})
		if err != nil {
			return err
		}
	}
...
func main() {
	...
	if *streaming {
		...
		RegisterStreamingGreeterServer(s, &server{})
	} else {
