...
DEFINE_string(offsetgen_filepath, "/px/offsetgen_offsets.json",
              "Path to the offsetgen file that contains the offsets of Go structs and functions. "
              "This file is generated by the offsetgen tool");
...
static void InitSymAddrs() {
  ParseGoOffsetgenFile(FLAGS_offsetgen_filepath);
  VLOG(1) << absl::Substitute("Parsed $0 struct and $1 function offsets from offsetgen file",
                              GetGoStructOffsets().size(), GetGoFunctionArgOffsets().size());
  return;
}
...
int UProbeManager::DeployGoUProbes(const absl::flat_hash_set<md::UPID>& pids) {
  ...
  for (const auto& [binary, pid_vec] : ConvertPIDsListToMap(pids)) {
    ...
    LOG(INFO) << "Go version=" << go_version << " for binary " << binary;
