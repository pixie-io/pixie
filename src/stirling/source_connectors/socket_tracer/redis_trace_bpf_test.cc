...
TEST_F(RedisTraceBPFTest, VerifyBatchedCommands) {
  ...
  constexpr std::string_view kRedisPodmanCmdTmpl =
      R"(podman run --rm --network=container:$0 redis bash -c "echo '$1' | redis-cli")";
...
TEST_F(RedisTraceBPFTest, VerifyPubSubCommands) {
  ...
  std::string redis_cli_cmd =
      absl::Substitute("podman run --rm --network=container:$0 redis redis-cli publish foo test",
...
TEST_F(RedisTraceBPFTest, ScriptLoadAndEvalSHA) {
  ...
  std::string script_load_cmd = absl::Substitute(
      R"(podman run --rm --network=container:$0 redis redis-cli script load "return 1")",
      container_.container_name());
  ...
  std::string evalsha_cmd = absl::Substitute(
      "podman run --rm --network=container:$0 redis redis-cli evalsha $1 2 1 1 2 2",
...
TEST_P(RedisTraceBPFTest, VerifyCommand) {
  ...
  std::string redis_cli_cmd =
      absl::Substitute("podman run --rm --network=container:$0 redis redis-cli $1",
