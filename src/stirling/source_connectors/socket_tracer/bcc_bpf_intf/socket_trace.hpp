...
inline std::string ToString(const socket_data_event_t::attr_t& attr) {
  return absl::Substitute(
      "[data ts=$0 conn_id=$1 protocol=$2 role=$3 dir=$4 ssl=$5 source_fn=$6 pos=$7 size=$8 "
      "buf_size=$9]",
...
inline std::string ToString(const conn_stats_event_t& event) {
  return absl::Substitute("[conn_stats wr_bytes=$0 rd_bytes=$1]", event.wr_bytes, event.rd_bytes);
}

inline std::string ToString(const close_event_t& event) {
  return absl::Substitute("[close wr_bytes=$0 rd_bytes=$1]", event.wr_bytes, event.rd_bytes);
}

inline std::string ToString(const conn_event_t& event) {
  return absl::Substitute("[open addr=$0]",
...
struct SocketDataEvent {
  ...
  std::string ToString() const {
    return absl::Substitute("attr:[$0] msg_size:$1 msg:[$2]", ::px::stirling::ToString(attr),
                            msg.size(), BytesToString<bytes_format::HexAsciiMix>(msg));
  }
