...
TEST(StringReaderTest, basic) {
  std::unique_ptr<pypa::Reader> reader = std::make_unique<StringReader>(
      "pl.DataFrame(table='cpu', select=['cpu0'])\\\n.Range(time='-2m')");
  EXPECT_EQ(1, reader->get_line_number());
  EXPECT_FALSE(reader->eof());
  EXPECT_EQ("pl.DataFrame(table='cpu', select=['cpu0'])\\\n", reader->next_line());
  EXPECT_EQ(1, reader->get_line_number());
  EXPECT_FALSE(reader->eof());
  EXPECT_EQ("pl.DataFrame(table='cpu', select=['cpu0'])\\", reader->get_line(1));
  EXPECT_EQ(1, reader->get_line_number());
...
TEST(StringReaderTest, pypa_syntax_error) {
  ...
  pypa::Lexer lexer(std::make_unique<StringReader>("queryDF = pl.DataFrame(table = 'cpu'"));
  pypa::AstModulePtr ast;
  pypa::SymbolTablePtr symbols;
  pypa::ParserOptions options;
  if (VLOG_IS_ON(1)) {
