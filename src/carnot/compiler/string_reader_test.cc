...
TEST(StringReaderTest, basic) {
  ...
  EXPECT_EQ("dataframe(table='cpu', select=['cpu0'])\\\n", reader->next_line());
  EXPECT_EQ(1, reader->get_line_number());
  EXPECT_FALSE(reader->eof());
  EXPECT_EQ("dataframe(table='cpu', select=['cpu0'])\\", reader->get_line(1));
  EXPECT_EQ(1, reader->get_line_number());
...
TEST(StringReaderTest, pypa_syntax_error) {
  ...
  pypa::Lexer lexer(std::make_unique<StringReader>("queryDF = dataframe(table = 'cpu'"));
  pypa::AstModulePtr ast;
  pypa::SymbolTablePtr symbols;
  pypa::ParserOptions options;
  if (VLOG_IS_ON(1)) {
