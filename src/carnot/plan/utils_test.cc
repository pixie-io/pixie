...
TEST(DataTypeToString, basic_tests) {
  EXPECT_EQ("bool", ToString(planpb::BOOLEAN));
  EXPECT_EQ("int64", ToString(planpb::INT64));
  EXPECT_EQ("float64", ToString(planpb::FLOAT64));
  EXPECT_EQ("string", ToString(planpb::STRING));
}
