...
class RuleExecutorTest : public OperatorTests {
 protected:
  void SetUp() override {
    ...
    compiler_state_ = std::make_unique<CompilerState>(std::move(rel_map), SensitiveColumnMap{},
                                                      info_.get(), time_now, 0, "result_addr",
                                                      "result_ssl_targetname", RedactionOptions{});
