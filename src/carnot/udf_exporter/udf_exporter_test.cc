...
TEST_F(UDFExporterTest, udf_proto) {
  ...
  // Contains a math op.
  auto udf_status = registry_info->GetUDF("pl.add", {types::FLOAT64, types::FLOAT64});
  EXPECT_OK(udf_status);
  EXPECT_TRUE(udf_status.ConsumeValueOrDie() == types::FLOAT64);

  // Contains a string op.
  udf_status = registry_info->GetUDF("pl.pluck", {types::STRING, types::STRING});
  EXPECT_OK(udf_status);
  EXPECT_TRUE(udf_status.ConsumeValueOrDie() == types::STRING);

  // Contains metadata ops.
  udf_status = registry_info->GetUDF("pl.upid_to_pod_id", {types::UINT128});
  EXPECT_OK(udf_status);
  EXPECT_TRUE(udf_status.ConsumeValueOrDie() == types::STRING);

  // Contains aggregate ops.
  auto uda_status = registry_info->GetUDA("pl.count", {types::BOOLEAN});
