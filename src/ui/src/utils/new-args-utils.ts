...
export function validateArgs(vis: Vis, args: Arguments): VizierQueryError|null {
  ...
  for (const { name, defaultValue, validValues } of vis.variables) {
    if (defaultValue === undefined && args[name] === undefined) {
...
export function argTypesForVis(vis: Vis): Record<string, string> {
...
export function argVariableMap(vis: Vis): Record<string, Variable> {
