...
class UPID {
 public:
  ...
  UPID() = default;
  UPID(uint32_t agent_id, uint32_t pid, int64_t ts_ns) {
    uint64_t upper = agent_id;
    upper <<= 32U;
    upper |= pid;
  ...
  uint32_t agent_id() const { return static_cast<uint32_t>(absl::Uint128High64(value_) >> 32U); }

  int64_t start_ts() const { return static_cast<uint32_t>(Uint128Low64(value_)); }

  absl::uint128 value() const { return value_; }

  std::string String() const { return absl::Substitute("$0:$1:$2", agent_id(), pid(), start_ts()); }
