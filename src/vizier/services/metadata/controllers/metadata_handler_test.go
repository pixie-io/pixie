...
func TestKubernetesNodeHandler(t *testing.T) {
	// Set up mock.
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	mockMds := mock_controllers.NewMockMetadataStore(ctrl)
	...
	mockMds.
		EXPECT().
		UpdateIPMap("127.0.0.1", "localhost").
		Times(1)

	// Create node object.
	addrs := make([]v1.NodeAddress, 2)
	addrs[0] = v1.NodeAddress{
		Type:    v1.NodeInternalIP,
		Address: "127.0.0.1",
	}
	addrs[1] = v1.NodeAddress{
	...
	ch := mh.GetChannel()
	msg := &controllers.K8sMessage{Object: &o, ObjectType: "nodes"}
	ch <- msg
}
