...
// QueryResultForwarderOld is responsible for receiving query results from agents
...
type QueryResultForwarderOld interface {
...
// QueryResultForwarderOldImpl implements the QueryResultForwarderOld interface.
type QueryResultForwarderOldImpl struct {
	// TODO(nserrino): Remove this reference when the batch API ReceiveAgentQueryResult
	// is deprecated in Kelvin and the query broker. QueryResultForwarderOldImpl will deal with
	// the client streams directly, rather than the Executors.
	executors map[uuid.UUID]Executor
}

// NewQueryResultForwarderOld creates a new QueryResultForwarderOld.
func NewQueryResultForwarderOld(queryExecutors map[uuid.UUID]Executor) QueryResultForwarderOld {
	return &QueryResultForwarderOldImpl{
		executors: queryExecutors,
	}
}

// ForwardQueryResult forwards the agent result to the client result stream.
func (f *QueryResultForwarderOldImpl) ForwardQueryResult(msg *carnotpb.TransferResultChunkRequest) error {
	...
	return fmt.Errorf("Error in QueryResultForwarderOldImpl: No query with ID %d found in executor map",
		msgQueryID.String())
}

// OptionallyCancelClientStream closes the client-side stream for the given queryID, if it still exists.
func (f *QueryResultForwarderOldImpl) OptionallyCancelClientStream(queryID uuid.UUID) error {
