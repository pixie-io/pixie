...
def _quiet_fake_which(program):
    return struct(_pl_fake_which = program)
...
def _find_cxx(repository_ctx):
    ...
    if hasattr(real_cxx, "_pl_fake_which"):
        real_cxx = repository_ctx.which(real_cxx._pl_fake_which)
        if real_cxx == None:
            find_cc(struct(
...
def _build_pl_cc_wrapper(repository_ctx):
    ...
    repository_ctx.template("extra_tools/pl_cc_wrapper", repository_ctx.attr._pl_cc_wrapper, {
        "{PL_REAL_CC}": repr(str(real_cc)),
        "{PL_REAL_CXX}": repr(str(real_cxx)),
    })
    return repository_ctx.path("extra_tools/pl_cc_wrapper")

def _needs_pl_cc_wrapper(repository_ctx):
