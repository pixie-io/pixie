...
def create_ar_archive(path, files):
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        subprocess.run(['ar', 'r', path] + content_paths, check=True)
...
def push_debs_to_gcs(gcs_prefix, deb_dir):
    ...
    gcs_paths = [
        f'{gcs_prefix}/{os.path.basename(deb_path)}' for deb_path in deb_files]
...
def main():
    ...
    with open(parsed_args.debs_json, 'r') as f:
        deb_path_to_checksum = json.load(f)

    with tempfile.TemporaryDirectory() as tmpdir:
        output_dir = f'{tmpdir}/output'
        os.makedirs(output_dir, exist_ok=True)

        for deb_path, checksum in deb_path_to_checksum.items():
