...
# Borrowed from envoy (8cd9c69): envoy/bazel/cc_wrapper.py
import contextlib
import os
import sys
import tempfile

pl_real_cc = {PL_REAL_CC}  # noqa: F821
pl_real_cxx = {PL_REAL_CXX}  # noqa: F821
...
def main():
    ...
    # consistent include paths, but Bazel only sets that option at compile
    ...
    if sys.argv[1:] == ["-E", "-xc++", "-", "-v"] and "clang" in compiler:
        os.execv(pl_real_cxx,
                 [pl_real_cxx, "-E", "-", "-v", "-no-canonical-prefixes"])
    ...
    # This check lets static linking against libstdc++ to be more
