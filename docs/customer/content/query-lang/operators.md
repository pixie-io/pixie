...
---
## From()
`From(String table, List<String> select=[])`

#### Example
```python
df = From(table="http_events", select=[
  "upid", 
  "http_resp_status", 
...
`From` accesses a table from memory and makes it available for Operators to actually process the data. This is the only operator 
...
`left_table.Join(Dataframe right_table, String type, Lambda cond, Lambda cols)`

#### Example

```python
table1 = From(table="http_events", select=["upid", "http_resp_latency_ns"])
table2 = From(table="process_stats", select=["upid", "cpu_ktime_ns"])
table1.Join(table2, type='inner', 
...
`Join` specifies an equijoin of two Tables, preserving the columns from each table specified in the `cols` argument. 

#### Arguments

* `right_table` (*type Dataframe*): the Table to Join with the table that the operator is called upon. The parent Dataframe on which the `Join` is called 
on is the left `Dataframe` while `right_table` represents the right table. 

* `type` (*type string*): defines the type of Join to be done. Allows ("inner", "outer", "left", "right")
