...
func init() {
	...
	flag.StringVar(&flagSpecMessageSize, "msg", "[1, 1024, 2048]", "The msg size spec")
...
func runWrkWithParams(duration, threads, connections, messageSize, thousandIters int64) {
	...
	wrkCommandArgs := fmt.Sprintf("--script report.lua -d %d -t %d -c %d %s", duration, threads, connections, hostStr)
